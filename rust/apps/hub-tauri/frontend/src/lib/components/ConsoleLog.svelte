<script lang="ts">
	...
	function levelBit(level: string): number {
		switch (level) {
			...
			case 'warn': case 'warning': case 'attention': return LOG_LEVEL_WARN;
	...
	function levelLabel(level: string): string {
		switch (level) {
			case 'warning': case 'attention': return 'WARN';
			case 'verbose': return 'DBG';
	...
	function levelColorStyle(level: string): string {
		switch (level) {
			...
			case 'warning':
			case 'warn':
			case 'attention': return `color: ${colors.warn}`;
	...
	function levelBadgeStyle(level: string): string {
		switch (level) {
			case 'error': return `background: ${colors.error}20; color: ${colors.error}; border: none`;
			case 'warning':
			case 'warn':
			case 'attention': return `background: ${colors.warn}20; color: ${colors.warn}; border: none`;
			case 'info': return `background: ${colors.info}20; color: ${colors.info}; border: none`;
			case 'debug':
			case 'verbose': return `background: ${colors.debug}20; color: ${colors.debug}; border: none`;
			default: return `background: ${colors.log}20; color: ${colors.log}; border: none`;
		}
	}
</script>
...
{:else}
	...
	>
		{#each filteredEntries as entry (entry.id)}
			<div class="flex items-start gap-2 px-3 py-0.5 hover:bg-zinc-900/50 border-b border-zinc-900">
				<span class="text-zinc-600 shrink-0">{formatTime(entry.timestamp)}</span>
				<span class="shrink-0 px-1 rounded text-[10px] font-medium uppercase whitespace-nowrap leading-relaxed min-w-[3.5ch] text-center" style={levelBadgeStyle(entry.level)}>
